...
@section('content')
    ...
    <div id="content" class="content">
		<div class="inner">
            ...
            <form id="agreeForm" action="/member/step01" method="post">
                @csrf
...
2010년 4월 23일부터 시행되던 종전의 약관은 본 약관으로 대체됩니다.
					...
					<div class="mt10">
						<label class="input-sp">
							<input type="checkbox" name="check1" class="checkedAll"/>
...
*이전 개인정보취급방침 보기(2014년 2월 1일부터 시행된 방침)
					...
					<div class="mt10">
						...
						<label class="input-sp">
							<input type="checkbox" name="check2"  class="checkedAll"/>
							<span class="input-txt">약관에 동의합니다.</span>
                        </label>
					</div>
				</div>
			...
			<div class="all-agree-box">
				<label class="input-sp">
					<input type="checkbox" name="checkAll" id="checkAll" class="checkedAll"/>
					<span class="input-txt">상위 이용약관 및 개인정보 취급방침에 모두 동의합니다.</span>
				</label>
			</div>
            </form>
			<div class="box-btn">
				<a id="next" href="#" class="btn-l">다음단계 (휴대폰인증)</a>
</div>
...
<script>
    ...
    $(function(){
        ...
        $("input[name='checkAll']").click(function(){
            const chkAll = $(this).is(":checked");
            if(chkAll) $(".checkedAll").prop('checked',true);
            else $(".checkedAll").prop('checked',false);
        });


        $("#next").click(function(){
            const chkVal = $('input:checkbox[name="check1"]').is(":checked");
            const chkVal2 = $('input:checkbox[name="check2"]').is(":checked");
            if(!chkVal){
                alert("이용약관에 동의해주세요");
                return false;
            }
